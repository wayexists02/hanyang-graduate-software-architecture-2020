...
public class GraphicUI extends JFrame {
	 ...
	 */
	public GraphicUI(Interacter interacter) {
		initialize();
	...
	private void clearRentals()
	{
        try {
            ...
            String result = interacter.clearRentals(code);
            
            clear();
            textArea.append(result);

        } catch (NumberFormatException exc) {
            showMessage("You should input a valid customer code!");
        }
	}
	...
	private void listVideos() {
		...
		textArea.append(interacter.listVideos());
	}

	private void listCustomers() {
        clear();
		textArea.append("List of customers\n");
		textArea.append(interacter.listCustomers());
	}
	...
	private void returnVideo() {
        try {
            ...
            if (videoTitle.length() == 0)
                showMessage("You shold input title of video!");
            else
	...
	private void rentVideo() {
        try {
            ...
            if (videoTitle.length() == 0)
                showMessage("You should input title of video!");
            else
                interacter.rentVideo(customerCode, videoTitle);
        } catch (NumberFormatException exc) {
            showMessage("You should input a valid customer code!");
        }
    ...
    private void registerUser() {
        try {
            ...
            if (name.length() == 0)
                showMessage("You should input a valid name!");
            else if (birthday.length() == 0)
                showMessage("You should input a valid birthday!");
            else
                interacter.registerCustomer(code, name, birthday);
